...
$username = isset($bid['user_name']) ? $bid['user_name'] : '';
$propertyname = isset($bid['property_name']) ? $bid['property_name'] : '';
$useraddress = isset($bid['user_address']) ? $bid['user_address'] : '';
$propertyaddress = isset($bid['propery_address']) ? $bid['propery_address'] : '';
$mobile = isset($bid['mobile']) && $bid['mobile'] != 0 ? $bid['mobile'] : '';
$todayDate = date('F d, Y');
$invoiceCode = '00'.$bid['id'];
$price = isset($bid['bid_amount']) ? $bid['bid_amount'] : '';
$currency = isset($bid['currency']) ? $bid['currency'] : '';
$pdes = isset($bid['pdes']) ? $bid['pdes'] : '';
...
$bidMade->invoice = 'invoice_'.$invoiceCode.'.pdf';
$invoiceUpdate = $controller->updateInvoice($bidMade);        
if($invoiceUpdate){
    $invoiceUrl = $extras->encryptQuery1(KEY_SALT, 'id', $bid['id'], 'invoice_view_bid.php');
